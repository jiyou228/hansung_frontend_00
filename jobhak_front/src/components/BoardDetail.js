      ...
      .then(
        axios.spread((bookmark, detail) => {
          if (bookmark.data.result)  {
            const bookmarkIds = bookmark.data.result.map((item) => item.postId.toString());
...
<div className="board_middle">
  <label className="content_div" dangerouslySetInnerHTML={{ __html: content }} />
</div>
